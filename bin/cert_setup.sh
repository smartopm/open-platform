...
if [ -z $1 ]; then
  ...
  docker-compose run --rm rails openssl aes-256-cbc -salt -pass file:config/credentials/development.key -in certs/dennis.dgdp.site/privkey.enc -out certs/dennis.dgdp.site/privkey.pem -pbkdf2 -a -d
elif [ $1 = "e" ]; then
  ...
  docker-compose run --rm rails openssl aes-256-cbc -salt -pass file:config/credentials/development.key -out certs/dennis.dgdp.site/privkey.enc -in certs/dennis.dgdp.site/privkey.pem -pbkdf2 -a -e
fi
