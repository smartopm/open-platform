...
module Mutations
  module Payment
    ...
    class PaymentCreate < BaseMutation
      ...
      def resolve(vals)
        ActiveRecord::Base.transaction do
          ...
          user.invoices.where('pending_amount > ?', 0).each do |invoice|
            ...
            remaining_amount = extract_payment(invoice, remaining_amount,
      ...
      def extract_payment(inv, amount, payment_type, transaction_id)
        payment_amount = amount > inv.pending_amount ? inv.pending_amount : amount
        payment = ::Payment.create(amount: payment_amount, payment_type: payment_type)
