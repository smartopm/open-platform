  ...
  module User
    ...
    class Import < BaseMutation
      ...
      def resolve(csv_string:)
        ...
        ActiveRecord::Base.transaction do
          csv.each_with_index do |row, index|
            ...
            user = current_user.enroll_user(name: name, email: email, phone_number: phone, state: state, user_type: user_type, expires_at: expires_at, request_note: notes)
      ...
      def user_already_present?(email, phone_list)
        ...
        ::User.joins(:contact_infos).where(contact_infos: { contact_type: "email", info: email }).or(
          ::User.joins(:contact_infos).where(contact_infos: { contact_type: "phone", info: phone_list })
        ).present?
      end
