...
export default function ProjectOverview({ data }){
  ...
  const formEntriesData = formData?.formUserProperties?.filter((f) => (FORM_FIELD_NAMES_TO_INCLUDE.includes(f.formProperty.fieldName)))
  ...
  return (
    <>
      ...
      />
     <Grid container>
       {Boolean(formEntriesData?.length)
