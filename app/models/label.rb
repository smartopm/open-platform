...
class Label < ApplicationRecord
  ...
  def self.with_users(com, limit, offset)
    Label.find_by_sql(["SELECT labels.id, labels.short_desc, labels.color, labels.description, COUNT(*) AS user_count
                FROM user_labels INNER JOIN labels ON user_labels.label_id = labels.id
                WHERE labels.community_id=?
                GROUP BY labels.id, labels.short_desc, labels.color, labels.description LIMIT ? OFFSET ? "] + [com, limit, offset])
  end
end
