...
namespace :import do
  desc 'Migrate Task Template'
  task :migrate_task_template, %i[community_name author_email] => :environment do |_t, args|
    abort('Community Name required') unless args.community_name & args.author_email
    ...
    abort('User not found') unless author
    ...
    ActiveRecord::Base.transaction do
      task_template.each do |t|
        ...
        t[:subTasks].each do |sub_task_title|
          community.notes.create(
            ...
            body: sub_task_title,
            category: 'template',
            description: 'A Template Sub Task',
            flagged: true,
            author_id: author[:id],
            completed: false,
            due_date: nil,
      ...
      if form_not_found
        ...
        form.update!(grouping_id: form.id) if form.save
      end
    end

    puts "Done Successfully. #{task_template.size} migrated. #{form_name} Created"
  end
end
