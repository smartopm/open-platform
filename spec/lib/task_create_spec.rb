...
RSpec.describe TaskCreate do
  ...
  context '#new_from_template' do
    ...
    it 'should create autogenerated To-DO tasks from template' do
      template_author = create(:user, community_id: user.community_id, role: user.role)
    ...
    it 'should assign a single user to sub-sub task when template has assignee' do
      template_author = create(:user, community_id: user.community_id, role: user.role)
      assignee = create(:user, community_id: user.community_id, role: user.role)
    ...
    it 'should assign multiple users to sub-sub task when template has assignee' do
      template_author = create(:user, community_id: user.community_id, role: user.role)
      assignee1 = create(:user, community_id: user.community_id, role: user.role)
      assignee2 = create(:user, community_id: user.community_id, role: user.role)
