...
RSpec.describe Mutations::Payment::PaymentPlanCreate do
  describe 'create for payment' do
    ...
    it 'creates a payment plan for one landparcel' do
      ...
      other_result = DoubleGdpSchema.execute(payment_plan_mutation, variables: variables,
                                                context: {
                                                current_user: admin,
                                                site_community: user.community,
                                                }).as_json
      expect(other_result.dig('errors', 0, 'message')).to eql 'Payment Plan for this landparcel already exist'
    end
  end
end
