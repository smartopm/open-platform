...
RSpec.describe Mutations::EntryRequest::InvitationCreate do
  describe 'create an invitation for a guest' do
    ...
    let!(:visitor) { create(:user, user_type: 'visitor', email: 'user@admin.com', community_id: user.community_id) }
    let!(:entry_request) { community.entry_requests.create!(name: 'John Doe', reason: 'Visiting', guest_id: visitor.id, user: admin) }
